    ...
    <section className="relative min-h-[85vh] md:min-h-screen overflow-hidden bg-gradient-to-br from-background via-primary/5 to-accent/5">
      ...
      <div className="relative z-10 container mx-auto px-4 py-8 md:py-12 lg:py-16">
        <div className="grid lg:grid-cols-2 gap-6 md:gap-8 lg:gap-12 items-center">
          ...
          {/* Left: server-rendered title & description for SEO (kept simple) */}
          <div className="animate-fade-in lg:-mt-36">
            <div className="inline-flex items-center space-x-2 px-3 py-1.5 md:px-4 md:py-2 bg-safety/10 rounded-full border border-safety/20 mb-4">
            ...
            <p className="text-base md:text-lg lg:text-xl text-muted-foreground mt-4">
            ...
            {/* Features Grid + Social Proof (server-rendered for SEO) */}
            <div className="mt-6">
              <div className="space-y-4 md:space-y-6 animate-fade-in lg:-mt-36">
                  ...
                  <div className="grid grid-cols-2 gap-2 md:gap-3">
                    {['Best Invisible Grills','Quality Safety Nets','Free Site Inspection','Professional Installation','Warranty-Backed Service','24/7 Support'].map((feature, index) => (
                      <div key={index} className="flex items-center space-x-2 text-xs md:text-base">
                        <svg className="h-3 w-3 md:h-4 md:w-4 text-accent flex-shrink-0" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 0C5.371 0 0 5.371 0 12s5.371 12 12 12 12-5.371 12-12S18.629 0 12 0zm-1.2 17.4l-5.6-5.6 1.68-1.68 3.92 3.92 7.12-7.12 1.68 1.68-8.88 8.8z"/></svg>
                        <span>{feature}</span>
                      </div>
                    ))}
                  </div>
                ...
                <div className="flex items-center gap-3 pt-3 border-t border-border/50">
                  <div className="flex items-center space-x-0.5">
                    {[...Array(5)].map((_, i) => (
                      <svg key={i} className="h-3 w-3 md:h-4 md:w-4 fill-safety text-safety" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 .587l3.668 7.431L24 9.748l-6 5.847L19.335 24 12 20.202 4.665 24 6 15.595 0 9.748l8.332-1.73z"/></svg>
                    ))}
                  </div>
          ...
          <div className="space-y-4 md:space-y-6 animate-fade-in" style={{ animationDelay: '200ms' }}>
            <div className="relative group rounded-xl md:rounded-2xl overflow-hidden shadow-strong">
              <div className="relative aspect-[16/9] md:aspect-[4/3] lg:aspect-[3/2]">
                <Image src="/images/hero-image.jpg" alt="Professional Invisible Grills and Safety Nets Installation" fill className="object-cover transition-transform duration-500 group-hover:scale-105" priority sizes="(max-width: 775px) 100vw, 50vw" quality={85} />
                <div className="absolute inset-0 bg-gradient-to-t from-black/20 to-transparent" />
              </div>
            </div>

            {/* Client-only consultation form */}
